...
int g_CntTime = 0;								// 時間（フレーム）カウンタ
int	g_Mode = MODE_LOGO_CREATOR;					// 起動時の画面を設定
...
void Update(void)
{
	...
	case MODE_LOGO_CREATOR:
		UpdateLogoCreator();// ロゴ（製作者）の更新
		break;
	...
	case MODE_TUTORIAL:		// チュートリ画面の更新
		UpdateTutorial();

		break;


	case MODE_OPTION:		// オプション画面の更新
		UpdateOption();
		break;
...
void Draw(void)
{
	...
	case MODE_TUTORIAL:		// チュートリ画面の描画
		DrawTutorial();

		break;


	case MODE_OPTION:		// オプション画面の描画
		DrawOption();
		break;
...
void SetMode(int mode)
{
	...
	case MODE_GAME:
		...
		PlaySound(SOUND_LABEL_BGM_bgm_battle);
		break;
